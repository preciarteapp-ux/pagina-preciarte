...
const WhatsAppButton = () => {
  const phoneNumber = "5564996259223";
  const message = encodeURIComponent("Estou no site e estou com dúvida sobre o PreciArte");
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${message}`;

  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3">
      {/* Tooltip "Dúvidas?" */}
      <div className="hidden sm:block animate-tooltip-fade bg-foreground text-background px-3 py-1.5 rounded-lg text-sm font-medium shadow-lg">
        Dúvidas?
      </div>
