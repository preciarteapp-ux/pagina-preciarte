...
const Pilares = () => (
  <section id="sistema" className="relative overflow-hidden bg-bf-ink py-[72px] lg:py-[130px]">
    ...
    <div className="relative max-w-bf-container mx-auto px-5 lg:px-8">
      <Reveal className="text-center">
        ...
        <p className={`${T.body} text-white/65 mt-5 max-w-[56ch] mx-auto`}>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5 mt-12 lg:mt-16">
        {PILARES.map((p, i) => (
          <Reveal key={p.nome} delay={i * 90}>
            <div className="h-full rounded-[24px] border border-bf-line bg-bf-surface p-7 lg:p-8 transition-colors duration-300 hover:border-bf-gold/40">
              <span className="inline-flex items-center justify-center w-[52px] h-[52px] rounded-full bg-gradient-to-br from-bf-gold to-bf-goldlight text-white">
                <Icon icon={p.icon} width={26} />
              </span>
              <h3 className={`${T.h3} text-white mt-5`}>{p.nome}</h3>
              <p className={`${T.cap} text-bf-gold mt-2`}>{p.papel}</p>
              <p className={`${T.small} text-white/65 mt-4`}>{p.texto}</p>
            </div>
          </Reveal>
        ))}
      </div>
