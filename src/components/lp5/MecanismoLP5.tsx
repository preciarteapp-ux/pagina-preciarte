import { T, Reveal, IconCircle } from './ui';

/* ── Diagramas ────────────────────────────────────────────────
   SVG inline (não <img>) para permitir animação de traço.
...
const box = { stroke: '#FB98B6', strokeWidth: 2, fill: 'none' };

function DiagramaHora() {
  return (
    <svg viewBox="0 0 360 360" className="w-full max-w-[320px] mx-auto lp5-draw" role="img" aria-label="Sua meta de ganho dividida pelas suas horas define o valor da sua hora, que entra em cada produto">
      ...
      <rect x="72" y="154" width="216" height="66" rx="16" stroke="#F66A93" strokeWidth="2" fill="rgba(246,106,147,.10)" />
...
function DiagramaPropagacao() {
  const ok = { stroke: '#2E7D5B', strokeWidth: 2, fill: 'rgba(46,125,91,.14)' };
  const bad = { stroke: '#C0392B', strokeWidth: 2, fill: 'rgba(192,57,43,.16)' };
  const pct = { fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 11 };
  const nos: [number, number, any, string, string, string][] = [
    [140, 34, ok, 'Caixa', '72%', '#6FD3A6'],
    [272, 86, ok, 'Tag', '64%', '#6FD3A6'],
    [272, 222, bad, 'Kit', '18%', '#F08379'],
    [140, 274, bad, 'Lembrança', '11%', '#F08379'],
    [8, 222, ok, 'Cartão', '68%', '#6FD3A6'],
    [8, 86, ok, 'Sacola', '61%', '#6FD3A6'],
  ];
  return (
    <svg viewBox="0 0 360 360" className="w-full max-w-[320px] mx-auto lp5-draw" role="img" aria-label="Ao atualizar o preço de um material, o custo de todos os produtos se corrige e a margem acusa quais ficaram no vermelho">
      <g>
      ...
      <rect x="112" y="146" width="136" height="68" rx="18" stroke="#F66A93" strokeWidth="2.5" fill="rgba(246,106,147,.12)" />
...
function DiagramaFinanceiro() {
  return (
    <svg viewBox="0 0 360 360" className="w-full max-w-[320px] mx-auto lp5-draw" role="img" aria-label="Ao marcar o pedido como pago, o lançamento entra no financeiro automaticamente">
      ...
      <rect x="60" y="98" width="72" height="24" rx="12" fill="rgba(46,125,91,.18)" stroke="#2E7D5B" strokeWidth="1.5" />
...
const BLOCOS = [
  {
    n: '01',
    titulo: 'A sua hora entra em cada peça, sozinha',
    corpo: <>Você define uma vez quanto quer ganhar e quantas horas trabalha. Todo produto que você cadastrar já nasce com <strong className="font-semibold text-white">o custo do seu tempo embutido</strong> — sem você precisar lembrar de somar.</>,
    diagrama: <DiagramaHora />,
  },
  {
    n: '02',
    titulo: 'Mudou um material, você descobre na hora quais preços ficaram errados',
    corpo: <>
  ...
  {
    n: '03',
    titulo: 'O pedido pago vira seu financeiro, sem você lançar nada',
    corpo: <>
...
export default function MecanismoLP5() {
  return (
    <section id="mecanismo" className="relative overflow-hidden bg-lp5n-900 py-[72px] lg:py-[120px]">
      <div
        className="absolute inset-x-0 top-0 h-[400px] pointer-events-none lp5-glow"
        style={{ background: 'radial-gradient(ellipse 50% 60% at 50% 0%, rgba(246,106,147,0.18), transparent 70%)' }}
      />

      <div className="relative max-w-lp5-narrow mx-auto px-5 lg:px-[120px]">
        <Reveal>
          <h2 className={`${T.h2} uppercase text-center text-white max-w-[900px] mx-auto`}>
            O PreciArte não começa perguntando o preço do seu produto.{' '}
            <span className="text-lp5-400">Começa perguntando o preço da sua hora.</span>
          </h2>

          <div className={`${T.body} text-white/[0.78] max-w-[65ch] mx-auto text-center mt-6 space-y-4`}>
            <p>
              A maioria das planilhas funciona assim: você soma o material, aplica uma porcentagem por cima e chama de
              preço. Seu tempo, quando entra, é um número que você mesma chutou.
            </p>
            <p>
              O PreciArte inverte isso. <strong className="font-semibold text-white">Você diz quanto quer ganhar por mês</strong> e
              quantas horas trabalha. O sistema soma seus custos fixos, divide pelas suas horas e chega ao valor real da
              sua hora. Depois <strong className="font-semibold text-white">injeta esse valor em cada produto que você cadastra</strong>, sozinho.
            </p>
            <p className="italic text-lp5-200">
              O seu pró-labore deixa de ser o que sobra e passa a ser o que entra na conta.
            </p>
          </div>
        </Reveal>

        <div className="mt-16 lg:mt-24 space-y-16 lg:space-y-[80px]">
          {BLOCOS.map((b, i) => (
            <Reveal key={b.n}>
              <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
                <div className={i % 2 === 1 ? 'lg:order-2' : ''}>{b.diagrama}</div>
                <div className={i % 2 === 1 ? 'lg:order-1' : ''}>
                  <IconCircle variant="solid">{b.n}</IconCircle>
                  <h3 className={`${T.h3} uppercase text-white mt-5`}>{b.titulo}</h3>
                  <div className={`${T.body} text-white/[0.78] mt-4 max-w-[65ch]`}>{b.corpo}</div>
                </div>
              </div>
            </Reveal>
          ))}
