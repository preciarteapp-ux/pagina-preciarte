...
export const calculateLoss = (a: QuizAnswers): QuizResult => {
  ...
  if (lossRatio > 0.45) {
    phrase = `Você trabalha muito. Você entrega com qualidade. Mas está deixando <strong>${brl(
      ...
      monthlyLoss * 12,
    )} por ano</strong> — na mesa. Por 12x R$ 10,64, o PreciArte elimina cada um desses pontos.`;
  } else if (lossRatio > 0.25) {
    phrase = `Com ${pedidos} pedidos por mês, você está perdendo <strong>${brl(
      monthlyLoss,
    )} todo mês</strong> em custos que não entram no preço e dinheiro que escapa sem controle. O PreciArte custa <strong>12x R$ 10,64</strong> — e se paga no primeiro pedido precificado certo.`;
  } else {
